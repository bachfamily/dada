...
* If you want to compile dada.music~, you need to have the GNU `GMP <https://gmplib.org/>`_ and `MPFR <https://www.mpfr.org/>`_ libraries installed. On OSX (except for Apple Silicon: see below for those), they are expected to be at /usr/local/opt/gmp and /usr/local/opt/mpfr respectively (modify the Xcode parameters if you have them elsewhere); if you install them with Homebrew, they should be in those places. On Windows, we use MPIR (https://www.mpir.org) instead of GMP, and Brian Gladman's Windows port of MPFR (https://github.com/BrianGladman/mpfr.git), which are expected to be installed in C:\Program Files\mpir and C:\Program Files\mpfr respectively. For linking with dada, we retargeted the MPIR and MPFR Visual Studio Projects to the Windows SDK 8.1, but we are not sure that this is necessary. Feel free to experiment!

* If you want to compile dada.graph, you need to have the `Boost <https://www.boost.org/>`_ library (version 1.71 works properly). On OSX (except for Apple Silicon: see below for those), it must be installed at /usr/local/opt/boost (modify the Xcode parameters if you have it elsewhere); if you install it with Homebrew, it should be in that place. On Windows, it must be installed at C:\Program Files\boost\boost_1_71_0. There is no need to build the Boost binaries.

* On an Apple Silicon machine, things are more complicated. For installing the libraries you need to have two versions of Homebrew installed, respectively for the ARM64 and Intel architectures. The ARM64 is the default one, which you can install by running in the terminal the commands provided in the Homebrew site home page. Once you've done this, you can run `brew install mpfr' and `brew install mpir' as usual. Similarly, do `brew install boost' if you need to.
  Once you've done this, you must install Homebrew for the Intel architecture. To do so, run `arch -x86_64 zsh' in a terminal. You should get no special notification, but this will open a zsh session under Rosetta 2. 
  Now you have to install Homebrew anew, but the x64 version will be installed in a different location: /usr/local/Homebrew/bin. To do so, follow the installation procedure explained in the home-brew website (https://brew.sh/).
  Now, go in /usr/local/Homebrew/bin and run `./brew install mpfr' and `./brew install mpir' (if you just call the system-wide brew, the arm64 version will be called, so that's not what you want to do). 
  This is also the time to install Boost: `./brew install boost' .
  At this point, you have two architecture-specific versions of the libraries installed. Now, for each library, you need to pack both architectures in a single fat binary. The arm64 version is in /opt/homebrew/lib, the x86 version is in /usr/local/opt/mpfr/lib and /usr/local/opt/gmp/lib . Notice that these steps don't concern boost, since we are not using the compiled parts of it. So we have to run the following lines:
    ...
    lipo /usr/local/opt/gmp/lib/libgmp.a /opt/homebrew/lib/libgmp.a -create -output /usr/local/opt/gmp/lib/libgmp.a
  We've only done it for the files that are used by dada, but of course you can repeat the process for any file you want.
  Anyway, the files that previously contained the x86 version now contain the fat binary, while the arm64 files have not been changed. This should allow you to compile everything.
