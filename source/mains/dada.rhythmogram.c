...
#include "dada.notation.spectral.h"
...
typedef struct _rhythmogram 
{
	...
	e_bach_mouse_cursors	mousecursor;
...
void rhythmogram_getdrawparams(t_rhythmogram *x, t_object *patcherview, t_jboxdrawparams *params);
...
/** Algorithms to handle frequency contents
	@ingroup	spectral
 */
typedef enum e_rhythmogram_algorithms
{
...
//	jbox_initclass(c, JBOX_COLOR | JBOX_FIXWIDTH | JBOX_FONTATTR);
	...
	class_addmethod(c, (method) rhythmogram_getdrawparams,	"getdrawparams", A_CANT, 0);
...
*/
	...
	CLASS_STICKY_ATTR(c,"category",0,"Display");
        ...
        CLASS_ATTR_DOUBLE_ARRAY(c, "domain", 0, t_rhythmogram, domain_ms, 2);
        CLASS_ATTR_STYLE_LABEL(c,"domain",0,"text","Displayed Domain");
        CLASS_ATTR_DEFAULT_SAVE_PAINT(c,"domain",0,"0 -1");
...
//				rhythmogram_set_numpoints(x, ceil((len/1000.) * x->sampling_frequency));
            ...
            // DGnew: this is never gonna happen
...
t_rhythmogram* rhythmogram_new(t_symbol *s, long argc, t_atom *argv){
    ...
    x->domain_ms[0] = 0; x->domain_ms[1] = -1;
	...
	if (llllobj_jbox_setup((t_llllobj_jbox *) x, 1, "444444444") != MAX_ERR_NONE) {
		error("Cannot find bach.llll: dada.rhythmogram can't be instatiated.");
		object_free_debug(x);
		return NULL;
	}
...
void rhythmogram_paint(t_rhythmogram *x, t_object *view){
    ...
    double domain_start_samp = x->domain_ms[0] >= 0 ? (x->domain_ms[0]/1000.)*x->sampling_frequency : 0;
    double domain_end_samp = x->domain_ms[1] >= 0 && x->domain_ms[1] >= x->domain_ms[0] ? (x->domain_ms[1]/1000.)*x->sampling_frequency : x->num_sampling_vals - 1;
    double domain_start_frame = x->domain_ms[0] >= 0 ? x->domain_ms[0] / (x->winsize /x->oversampling) : 0;
    double domain_end_frame = x->domain_ms[1] >= 0 && x->domain_ms[1] >= x->domain_ms[0] ? x->domain_ms[1] / (x->winsize/x->oversampling) : x->num_windows_in_history;
	...
	if (x->mode == k_MODE_ONSET_WAVE) {
		...
		if (x->num_sampling_vals > 0) {
			...
			for (i = 1; i < x->num_sampling_vals; i++){
				double xx = rescale(i, domain_start_samp, domain_end_samp, 0, rect.width);
		...
		t_rect src, dst;
		src.x = domain_start_frame;
		src.y = 0;
        src.width = domain_end_frame - domain_start_frame; // x->num_windows_in_history;
		...
		// for ts detector
		long curr_mouseover_frame_num = rescale(x->curr_mouse_pt.x, 0, rect.width, domain_start_frame, domain_end_frame); // 0-based
		double left, frameleft = rescale(curr_mouseover_frame_num, domain_start_frame, domain_end_frame, 0, rect.width);
...
//				double xx = rescale(frame_num, 1, x->num_windows_in_history+1, 0, rect.width);
                double xx = rescale(frame_num-1, domain_start_frame, domain_end_frame, 0, rect.width);
...
//				double xx = rescale(frame_num, 1, x->num_windows_in_history+1, 0, rect.width);
				...
				if (x->mode == k_MODE_TS_DETECTOR && frame_num == curr_mouseover_frame_num + 1 && (modifiers & eControlKey)) {
                    ...
                    write_text_standard(g, jf_legend, color, text, text_x, text_y, rect.width - text_x, rect.height - text_y);
				}
			...
			if (modifiers & eControlKey) {
				...
				if (sols_elem) {
					...
					if (true) { 
						...
						write_text_standard(g, jf_legend, x->j_hintcolor, "  Admissible istantaneous configurations:", left, curr_y, width, rect.height - curr_y);
						curr_y += vstep;
						for (config = sols->l_head; config; config = config->l_next) {
							...
							write_text_standard(g, jf_legend, x->j_hintcolor, text, left, curr_y, width, rect.height - curr_y);
							curr_y += vstep;
						}
					}
				}
...
//					double xe = rescale(end_fr + 1, 1, x->num_windows_in_history+1, 0, rect.width);
                    double xs = rescale(start_fr-1, domain_start_frame, domain_end_frame, 0, rect.width);
                    double xe = rescale(end_fr, domain_start_frame, domain_end_frame, 0, rect.width);
					...
					write_text_standard(g, jf_legend, x->j_hintcolor, text, xs, v_cursor + 2.5, xe - xs - 2, rect.height - v_cursor);
...
//								double tick_x = rescale(ms, 0, x->num_windows_in_history * x->winsize/x->oversampling, 0, rect.width);
                                double tick_x = rescale(ms, domain_start_samp * (1000./x->sampling_frequency), domain_end_samp * (1000./x->sampling_frequency), 0, rect.width);
				...
				if (x->curr_mouseover_tsd_segment && x->j_mouse_is_over) {
					...
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda1, x1, y1, maxwidth + 20, txtheight1 + 20);
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda2, x1, y1 + txtheight1, maxwidth + 20, txtheight2 + 20);
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda3, x1, y1 + txtheight1 + txtheight2, maxwidth + 20, txtheight3 + 20);
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda4, x1, y1 + txtheight1 + txtheight2 + txtheight3, maxwidth + 20, txtheight4 + 20);
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda5, x1, y1 + txtheight1 + txtheight2 + txtheight3 + txtheight4, maxwidth + 20, txtheight5 + 20);
					write_text_standard(g, jf_legend, x->j_hintcolor, legenda6, x1, y1 + txtheight1 + txtheight2 + txtheight3 + txtheight4 + txtheight5, maxwidth + 20, txtheight6 + 20);
				}
	...
	if (x->show_hint && x->j_mouse_is_over && x->mode != k_MODE_TS_DETECTOR){
		...
		if (!x->realtime && (x->mode == k_MODE_RHYTHMOGRAM || x->mode == k_MODE_AUTOCORRELOGRAM)){
            if (x->curr_hint_ms == DBL_MAX)
                snprintf_zero(legend1, 1000, "(outside analysis region)");
            else
                snprintf_zero(legend1, 1000, "%.3f s (frame %ld)", x->curr_hint_ms / 1000., x->curr_hint_frame + 1);
			...
			double ms = rescale(x->curr_mouse_pt.x, 0, x->width, domain_start_samp * (1000./x->sampling_frequency), domain_end_samp * (1000./x->sampling_frequency));
			this_idx_for_rhythmoscope_circle = round(rescale(x->curr_mouse_pt.x, domain_start_samp * (1000./x->sampling_frequency), domain_end_samp * (1000./x->sampling_frequency), 0, x->num_sampling_vals));
		...
		write_text_standard(g, jf_legend, x->j_hintcolor, legend1, x1, y1, txtwidth + 20, txtheight + 20);
		if (legend2[0])
			write_text_standard(g, jf_legend, x->j_hintcolor, legend2, x1, y1 + txtheight - txtheight2, txtwidth + 20, txtheight + 20);
	}
...
void rhythmogram_mousemove(t_rhythmogram *x, t_object *patcherview, t_pt pt, long modifiers) {
	...
	if (x->mode == k_MODE_TS_DETECTOR && !(modifiers & eControlKey)) {
		...
		if (cfg && whatclicked) {
            if (x->mousecursor != BACH_CURSOR_RESIZE_LEFTRIGHT) {
                x->mousecursor = BACH_CURSOR_RESIZE_LEFTRIGHT;
				jmouse_setcursor(patcherview, (t_object*) x, JMOUSE_CURSOR_RESIZE_LEFTRIGHT);
			}
		} else {
            if (x->mousecursor != BACH_CURSOR_DEFAULT) {
				jmouse_setcursor(patcherview, (t_object*) x, JMOUSE_CURSOR_ARROW);
                x->mousecursor = BACH_CURSOR_DEFAULT;
			}
		}
	} else {
        if (x->mousecursor != BACH_CURSOR_DEFAULT) {
	...
	if (x->show_hint){
		...
		if (x->mode == k_MODE_RHYTHMOSCOPE || x->mode == k_MODE_AUTOCORRELATION){
            ...
            x->curr_hint_frame = x->tags_size <= 0 ? 0 : rescale(pt.x, 0, x->width, domain_start_frame, domain_end_frame);
//			x->curr_hint_frame = x->tags_size <= 0 ? 0 : CLAMP(floor(rescale(pt.x, 0, x->width, start, end)), start, end);
			x->curr_hint_ms = x->tags && x->tags_size > 0 && x->curr_hint_frame < x->tags_size ? x->tags[x->curr_hint_frame] : DBL_MAX;
		} else if (x->mode == k_MODE_ONSET_WAVE){
			x->curr_hint_frame = 1;
			x->curr_hint_ms = 0;
//		} else if (x->mode == k_MODE_PARTIAL_TRACKING || x->mode == k_MODE_TS_DETECTOR){
		} else if (x->mode == k_MODE_PARTIAL_TRACKING){
            long start = domain_start_frame;
            long end = MIN(x->tags_size - 1, domain_end_frame);
			x->curr_hint_frame = x->tags_size <= 0 ? 0 : CLAMP(floor(rescale(pt.x, 0, x->width, domain_start_frame, domain_end_frame)), start, end);
...
//	long val = CLAMP(round(rel_xpos * x->num_windows_in_history), 0, x->num_windows_in_history);
    double domain_start_frame = x->domain_ms[0] >= 0 ? x->domain_ms[0] / (x->winsize /x->oversampling) : 0;
    double domain_end_frame = x->domain_ms[1] >= 0 && x->domain_ms[1] >= x->domain_ms[0] ? x->domain_ms[1] / (x->winsize/x->oversampling) : x->num_windows_in_history;
    long val = round(rel_xpos * (domain_end_frame - domain_start_frame) + domain_start_frame);
	return val;
}
