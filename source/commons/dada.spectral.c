/**
	@file
	dada.spectral.c
	Functions to handle spectral stuff for dada.rhythmogram

	by Daniele Ghisi
*/


#include "dada.spectral.h"
...
void take_amplitude_into_account(double bpm, double ampli, char consider_phase, double phase, double *sampled_amplitudes, double *sampled_phases, long num_sampling_points,
    ...
    if (bpm >= min_bpm && bpm <= max_bpm){
        ...
        if (shape_of_contributions == k_SHAPE_PUNCTUAL) {
            max_idx = min_idx = CLAMP(round(rescale(bpm, min_bpm, max_bpm, 0, num_sampling_points - 1)), 0, num_sampling_points - 1);
        } else {
            min_idx = CLAMP(floor(rescale(bpm-decay_bpm, min_bpm, max_bpm, 0, num_sampling_points - 1)), 0, num_sampling_points - 1);
            max_idx = CLAMP(ceil(rescale(bpm+decay_bpm, min_bpm, max_bpm, 0, num_sampling_points - 1)), 0, num_sampling_points - 1);
        }
                        ...
                        if (onset >= 0 && onset <= max_representable_onset){
                            ...
                            if (chord_energy > 0){
                                ...
                                if (shape_of_contributions == k_SHAPE_PUNCTUAL) {
                                    ...
                                    long bin_idx = CLAMP(round(onset/sampling_step_ms), 0, num_sampling_points - 1);
                                    ...
                                    long j, center_bin_idx = floor(onset/sampling_step_ms);
...
long mc_to_pitch_class(long mc)
{
    return ((long)round((mc % 1200)/100.)) % 12;
}
                                     ...
                                     char blackandwhite_chroma, char use_decay, double win_start, double win_end, char win_direction)
{
    ...
    for (elem = notes_laced_llll->l_head; elem; elem = elem->l_next) {
        ...
        if (blackandwhite_chroma) {
            if (contribution > chroma[mc_to_pitch_class(this_mc)])
                chroma[mc_to_pitch_class(this_mc)] = contribution;
        } else {
            chroma[mc_to_pitch_class(this_mc)] += contribution;
        }
    }
                                ...
                                char snap_to_nearest_onset, double median_filter_window_ms, double delete_softer_harmonic_changes_inside_win_ms, t_llll **debug)
{
    ...
    if (!notes_laced_llll || !notes_laced_llll->l_tail)
        ...
        for (elem = filtered_probabilities->l_head, i = 0; i < filtered_probabilities->l_size; elem = elem ? elem->l_next : NULL, i++)  {
...
void gathered_syntax_to_onset_wave(t_llll *chords_in_gathered_syntax,
    ...
    // 3. build an interlaced list containing (onset voice_number pitch dur vel chord_idx) (onset voice_number pitch dur vel chord_idx) (onset voice_number pitch dur vel chord_idx)... for each note
                    ...
                    if (chllll && chllll->l_head && chllll->l_head->l_next && is_hatom_number(&chllll->l_head->l_hatom)){
                        ...
                        for (noteselem = chllll->l_head->l_next; noteselem; noteselem = noteselem->l_next){
                            if (hatom_gettype(&noteselem->l_hatom) == H_LLLL) {
                                ...
                                llll_appenddouble(notellll, onset);
                                llll_appendlong(notellll, i); // voice number
                                llll_appenddouble(notellll, mc);
                                llll_appenddouble(notellll, dur);
                                llll_appendlong(notellll, vel);
                                llll_appendlong(notellll, chord_idx);
                                llll_appendllll(laced_llll, notellll);
                            }
                        }
                    }
                }
        ...
        for (lacedelem2 = lacedelem1->l_next; lacedelem2; lacedelem2 = lacedelem2->l_next){
            ...
            if (onset2 > onset1 && onset2 - onset1 < outer_onset_threshold &&
                ...
                if (note_neighborhood_to_consider <= 0 || abs(idx1 - idx2) <= note_neighborhood_to_consider) {
                    ...
                    //                    if (!(idx2 == idx1 + 1 && voicenum1 == voicenum2))
                    //                        couple_importance *= 0.7 * rescale_with_slope(CLAMP(onset2 - onset1, 0, outer_onset_threshold), 0, outer_onset_threshold, 1, 0, 0, true); // CONSTANT TO GAUGE
    ...
    if (use_masking_detection) {
        ...
        if (DEBUG)
            post("START of masking detection");
        ...
        for (elemA = couples->l_head, i = 1; elemA; elemA = elemA->l_next, i++)
            llll_appendlong(indexes, i, 0, WHITENULL_llll);
        ...
        lists_to_sort[1] = indexes;  // (1 2 3 4 5 6 7 8 9)
        ...
        indexes = lists_to_sort[1];  // (7 2 4 1 5 6 8 3 9)
        ...
        for (elemA = indexes->l_head, elemB = couples->l_head; elemA && elemB; elemA = elemA->l_next, elemB = elemB->l_next) {
            hatom_change_to_obj(&elemA->l_hatom, elemB);
            elemB->l_thing.w_obj = (t_object *)elemA;
        }
        // now indexes has (&g &b &d &a &e &f &h &c &i)
        
        // Finally, we sort again the couples by onset
        llll_inplacesort(couples, (sort_fn) llll_sort_by_first);
        
        for (elem_ordered_by_importance = indexes->l_head; elem_ordered_by_importance; elem_ordered_by_importance = elem_ordered_by_importance->l_next) {
                ...
                if (superposition > 0 - extend_masking_ms) {
                    ...
                    if (must_drop) {
                        ...
                        if (60 * (1000/B_delta_onset) >= debug_clip_min_bpm && 60 * (1000/B_delta_onset) <= debug_clip_max_bpm) {
                            ...
                            ezmidicents2notename(5, B_mc1, k_NOTE_NAMES_ANGLOSAXON, true, &B_mc1_str, 2);
                            ezmidicents2notename(5, B_mc2, k_NOTE_NAMES_ANGLOSAXON, true, &B_mc2_str, 2);
                            ezmidicents2notename(5, A_mc1, k_NOTE_NAMES_ANGLOSAXON, true, &A_mc1_str, 2);
                            ezmidicents2notename(5, A_mc2, k_NOTE_NAMES_ANGLOSAXON, true, &A_mc2_str, 2);
        ...
        if (DEBUG) { //verbose
            ...
            for (elemA = couples->l_head; elemA; elemA = elemA->l_next) {
                ...
                if (60 * (1000/A_delta_onset) >= debug_clip_min_bpm && 60 * (1000/A_delta_onset) <= debug_clip_max_bpm) {
                    ...
                    ezmidicents2notename(5, A_mc1, k_NOTE_NAMES_ANGLOSAXON, true, &A_mc1_str, 2);
                    ezmidicents2notename(5, A_mc2, k_NOTE_NAMES_ANGLOSAXON, true, &A_mc2_str, 2);
                    post("- (voice %ld, onset %.0fms, %s) – (voice %ld, onset %.0fms, %s) – delta onset: %.0fms – importance %.2f – gain %.2f", A_voice1, A_onset1, A_mc1_str, A_voice2, A_onset2, A_mc2_str, A_delta_onset, A_couple_importance, A_couple_gain);
                    bach_freeptr(A_mc1_str);
                    bach_freeptr(A_mc2_str);
                }
            }
...
void bach_fm_acf(int nfft, const kiss_fft_cpx *fin, double sample_rate, double *freq_mapped_acf, char rectify, char filter_last_peak)
{
    ...
    double *idx_for_interp = (double *) bach_newptr(nfft * sizeof(double)); // indexes of the bins in which we will have to interpolate the time-lagged acf
...
{
    double phase1_wk = positive_fmod(phase1, modulo);
    double phase2_wk = positive_fmod(phase2, modulo);
